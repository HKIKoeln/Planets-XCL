...
try
 ...
 // set error handler:
  ...
  if(choice==1)
    ...
    #endif
    #ifdef _STDVERSION
    pParser->loadGrammar("c:\\comparator\\res\\schemas\\xcdl\\XCDLCore.xsd", 1, true);
    #endif
...
for(int c=0; c<=2 && pErr->error!=-1; c++)
   ...
   try
     ...
     else if(c==2 && choice==2)  // validate PCR file
       {
       PCREntityResolver *pPCRER= new PCREntityResolver();
       pParser->setEntityResolver(pPCRER);
       filename= pIP->PCRFile;
       #ifdef DEBUG_XMLVALIDATION
       std::cout<<"Parsing file: "<<std::string(filename)<<" for validation."<<std::endl;
       #endif
       pParser->parse(pIP->PCRFile);
       delete pPCRER;
       }
     else break;
     }
