...
class XCELInterpretHexValuesMethod:public XCELProcessingMethod{
public:
   XCELInterpretHexValuesMethod(){trigger="interpretHexValues";}
~XCELInterpretHexValuesMethod(){}
   ...
   //     const QString& getTrigger();
    ...
    private:
        XCLParsingExpression* getCMap(XCELReader* reader, QString fontIdentifier);
        ushort getCMapIndexLength(XCLParsingExpression* cMap);
        QString getValue(XCLParsingExpression* cMap, QString key);
        QString getInterpretationASCII(QString decStr);
        QString getUTF8(QString unicodeStr);
        QString getEncoding(QString unicodeStr, QString encoding);

        XCLParsingExpression* getFontObject(XCELReader* reader, QString fontIdentifier);
        XCLParsingExpression* getChild(XCLParsingExpression* pExpr, QString identifier);
        void printFontObject(XCELReader* reader);
