...
/*class XCELSetMultipleProcessingMethod :public XCELProcessingMethod
{
public:
~XCELSetMultipleProcessingMethod(){};
XCELSetMultipleProcessingMethod(){trigger="setMultiple";}
...
public:
	...
	BOOL preprocess(XCLSyntaxItem& grammar, const XCLSyntaxProcess& preprocessing);
    //Processing: can modify the grammar or the binary
  	BOOL process(	XCLSyntaxExpression* expr,
                                QVector<XCLProcessParameter> param,
                                FileParserState& state,QHash<QString,IndexedVector*> vectorHash);
 	BOOL process( 	QByteArray* ba,   			//the complete file
				QVector<XCLProcessParameter> param,
                                FileParserState& state,QHash<QString,IndexedVector*> vectorHash);
